...
export async function connectLogin(sdsConnection: SDSConnection | undefined, conn: config.Connection): Promise<SDSConnection> {
    return new Promise<SDSConnection>(async (resolve, reject) => {
        ...
        try {
            ...
            await connection.connect(conn.clientName ? conn.clientName : "node-documents-scripting", conn.server, conn.port, conn.tls, { StartTls: conn.startTls, TrustedCas: conn.trustedCas, InitPDMeta: false });
        } catch (err) {
            return reject(err);
        }
        try {
